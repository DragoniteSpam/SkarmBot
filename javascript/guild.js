...
const GUILD_DIR = `../skarmData/guilds`;
...
const linkFunctions = function (guild) {
    ...
    guild.notify = function (client, notification, eventObject) {
        ...
        if (notification === Constants.Notifications.NAME_CHANGE) {
            ...
            let oldName = "";
            try {
                let user = User.get(eventObject.user.id)
                oldName = user.previousName;
                user.previousName = undefined;   // clear field after use
            } catch (error) {
                Skarm.spam("Caught a name change event that threw an error");
                console.error(error);
                return -1;
            }
...
class Guild {
    ...
    static async save() {
        try {
            ...
            await Encrypt.write(guilddb, JSON.stringify(Guild.guilds));
            ...
            for (let id in Guild.guilds) {
                await Encrypt.write(
                    `${GUILD_DIR}/${id}.penguin`,
                    JSON.stringify(Guild.guilds[id])
                );
            }
